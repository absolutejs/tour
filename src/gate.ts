...
export const useTourGate = (options?: TourGateOptions) => {
  ...
  const write = (slug: string, state: TourGateState) => {
    try {
      local()?.setItem(`${storageKey}.gate.${slug}`, JSON.stringify(state));
    } catch (err) {
  ...
  const shouldAutoPlay = (tutorial: Tutorial, routePath: string) => {
    ...
    if (trigger.onRoutePrefix && !routePath.startsWith(trigger.onRoutePrefix)) {
      return false;
    }
    ...
    if (state.dismissals >= (trigger.maxDismissals ?? DEFAULT_MAX_DISMISSALS)) {
      return false;
    }
    if (trigger.firstVisitOnly && (state.completedAt || state.lastAutoPlayAt)) {
      return false;
    }
