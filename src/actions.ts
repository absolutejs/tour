...
export type TourActionRegistry = {
  ...
  registerAll: (handlers: Record<string, TourActionHandler>) => () => void;
...
const builtinActions: Record<string, TourActionHandler> = {
  click: (context) => {
    const selector =
      typeof context.args.selector === "string" ? context.args.selector : null;
    const el = selector ? document.querySelector(selector) : context.target;
  ...
  wait: (context) =>
    tourWait(
      typeof context.args.ms === "number" ? context.args.ms : DEFAULT_WAIT_MS,
      context.signal,
    ),
};
...
export const runTourActions = async (
  ...
  for (const ref of refs) {
    ...
    if (context.signal.aborted) return;
    const handler = registry.resolve(ref.action) ?? builtinActions[ref.action];
    if (!handler) {
